...
async function handleAntideleteCommand(sock, chatId, message, match) {
    ...
    if (!match) {
        return sock.sendMessage(chatId, {
            text: `*ANTIDELETE SETUP*\n\nCurrent Status: ${config.enabled ? '✅ Enabled' : '⚠️ Disabled'}\n\n*.antidelete on* - Enable\n*.antidelete off* - Disable`
        });
    }
...
async function storeMessage(message) {
    try {
        ...
        if (message.message?.conversation) {
            ...
            const buffer = await downloadContentFromMessage(audioMsg, 'audio');
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        let text = `*⛔️ ANTIDELETED MESSAGE ⛔️*\n\n` +
        ...
        if (original.mediaType && fs.existsSync(original.mediaPath)) {
            const mediaOptions = {
                caption: `*Deleted ${original.mediaType === 'ptt' ? 'Voice Note' : original.mediaType}*\nFrom: @${senderName}`,
                mentions: [sender]
            };
